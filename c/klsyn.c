...
/* Needed for Windows systems, where MinGW doesn't have strsep() */
/* Implementation found at http://web.archiveorange.com/archive/v/yWJfLhLak3mrcWaNVUyb */
/* TODO: implement read_klt in Python and remove this code */
...
//}
//
//void read_klt(char *fname)   {
//    FILE *fp;
//    int nf, np, value;
//    char line[10000],word[50];
//    char *token, *linep;
//    
//    if ((fp = fopen(fname,"r")) == NULL) {
//        printf("\tUnable to find %s, exiting.\n",fname);
...
//    while (fgets(line, sizeof line, fp) != NULL) {
//
//        if ((sscanf(line,"<%s",word)==1) && (strcmp(word,"config>")==0)) {
//                /* read until </config> */
//        
//            while (fgets(line,sizeof line, fp)!=NULL) {  // get next line
//                if ((sscanf(line,"</%s",word)==1) && (strcmp(word,"config>")==0)) break;  /* go back to while */
//                if (sscanf(line,"%s %d",word,&value)==2) {
//                    defval[find_config_loc(word)] = value;
//                }
//            }
//            OUTSELECT= defval[find_config_loc("os")];
//            RANSEED  = defval[find_config_loc("rs")];
...
//        }
//
//        if (strcmp(word,"pars>")==0) {
//            nvar=0;  /* this is a global used by other routines */
//            while (fgets(line,sizeof line, fp)!=NULL) {
//                linep = strdup(line);
//                nf=0;
//                while ((token = strsep(&linep, "\t")) != NULL) {
//                    if (strcmp("<time>",token)==0) break;
//                    if (sscanf(token,"</%s",word)==1) {
//                        if (strstr(word,"pars")!=NULL) printf("\t%d parameters are time-varying.\n", nvar) ;
//                        break;
//                    }
//                    if (sscanf(token,"<%s",word)==1) {
//                        word[2]=0;
//                        np = find_config_loc(word);
//                        nvar++;
//                        cv[np]=VARRIED;
//                        continue;
//                    }
